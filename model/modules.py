...
class AxialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        N, W, C, H = x.shape
        x = x.contiguous().view(N * W, C, H) #1024 x 64 x 1024

        # Transformations
        qkv = self.bn_qkv(self.qkv_transform(x)) #1024 x 128 x 1024
        q, k, v = torch.split(qkv.reshape(N * W, self.groups, self.group_planes * 2, H), 
                              [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=2) #k 1024x1x32x1024  q 1024 x 1 x 32 x 1024 v 1024 x 1 x 64 x 1024

        # Calculate position embedding
        all_embeddings = torch.index_select(self.relative, 1, self.flatten_index).view(self.group_planes * 2,
                                                                                       self.kernel_size,
                                                                                       self.kernel_size)  # (128,400,400)
        q_embedding, k_embedding, v_embedding = torch.split(all_embeddings,
                                                            [self.group_planes // 2, self.group_planes // 2,
                                                             self.group_planes], dim=0)   #k 32x400x400 q 32x400x400 v 64x400x400
        #1024 x 1 x 32 x 1024   |    32x400x400

        
        # ######## modificifation of original code: resizing of q -------------------------------------------------------------------
        # # Specify the desired output size
        # desired_size = (32, 64)
        # # Resize the tensor using interpolate
        # q = F.interpolate(q, size=desired_size, mode='bilinear', align_corners=False)
        # k = F.interpolate(k, size=desired_size, mode='bilinear', align_corners=False)
...
class Encoder(nn.Module):
    def __init__(self, input_size, in_chans=1, embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8], depths=[2, 2, 2, 2]):
        super(Encoder,self).__init__()
        self.depths = depths
        self.embed_dims = embed_dims

        # patch_embed
        self.patch_embed1 = OverlapPatchEmbed(img_size=input_size, patch_size=7, stride=4, in_chans=in_chans,   #Original stride of the code = 4, change it because desired embedding dim (2,64,64,64)
