...
### GENERATE 
def render(x=0.0,y=0.0,z=0.5,r=0.0,obj="Z"):
    path = "/code/datasets/feature_generation_yaml/"
    yaml_file="/code/datasets/feature_generation_yaml/base.yaml"
    print(path)
    with open(yaml_file) as f:
        text = f.read()
    
    text_obj="/code/datasets/xml/obj/Z.obj"
    text_x = "displacement_x: 0.0"
    text_y = "displacement_y: 0.0"
    text_z = "displacement_z: 1.0"
    rot_z="rot_degrees_z: 0.0"

    new_text_obj = "/code/datasets/xml/obj/{}.obj".format(obj)
    new_text_x = "displacement_x: " + str(x)
    new_text_y = "displacement_y: " + str(y)
    new_text_z = "displacement_z: " + str(z)
    new_rot_z="rot_degrees_z: {}".format(r)
    ...
    new_text = new_text.replace(rot_z,new_rot_z)
...
def reconstruct(hdf5_file,obj_name,f_name):
    ...
    npy_file = "/code/datasets/train/feature/{}/{}".format(obj_name, f_name)
    g=reduce_image(images)

    #Save npy file
    print(g.shape)  
    np.save(npy_file, g)
    print(os.listdir("/code/datasets/train/feature/{}/".format(obj_name)))
            ...
            for j in range(0,19):
                for e in range(0,6):
                    for d in range(0,1):
                        ...
                        f_name = "x_{}_y_{}_z_{}_r_{}.npy".format(x,y,z,r)
                        #file_with_extension = f_name + ".npy"  # Assuming the file has a .txt extension
                        directory = "/code/datasets/train/feature/{}/{}".format(obj_name,f_name)
                        print(directory)
                        ...
                        #Render scene                
                        hdf5_file,created_folder = render(x,y,z,r,obj_name) #=args[2]
                        print("hdf5_file")
                        print(hdf5_file)
                        ...
                        print(os.listdir("/code/datasets/feature_generation_yaml/"))
                        r=r+30
                    z = z + 0.25
                    r=0.0
                x= x + 0.05
